...
class Digital5View extends Ui.WatchFace {
    ...
    var TimeY = 70;
    var DateY = 35;
    var DMode; //Debbuging Mode
    var DHour; //Debugging Hour
    var DMinute; // Debugging Minute

    enum { WOMAN, MEN }
    enum { UPPER_LEFT, UPPER_RIGHT, LOWER_LEFT, LOWER_RIGHT, BOTTOM_FIELD, CENTER_CENTER, UPPER_CENTER, TOP_TOP}
    enum { M, I, K, B, C, F }
    ...
    var upperLeftField, upperRightField, lowerLeftField, lowerRightField, bottomField, topField;
    ...
    function onUpdate(dc) {
        ...
        //Debugging
        DMode           	  = false; //Debugging Mode, force hardcoded values
        var DSteps = 9009;
        var DKcal = 2111;
        var Dbpm = 156; 
        var DBatt = 19; 
        ...
        secondsAlwaysOn           = false; //App.getApp().getProperty("SecondsAlwaysOn");
        lcdFont                   = true; //App.getApp().getProperty("LcdFont");
        lcdFontDataFields         = true; //App.getApp().getProperty("LcdFontDataFields");
        showLeadingZero           = true; //App.getApp().getProperty("ShowLeadingZero");
        ...
        hr                        = hrHistory.next();
        while(hr.heartRate == Act.INVALID_HR_SAMPLE || hr == null) {hr = hrHistory.next();}
        bpm                       = hr.heartRate > 0? hr.heartRate : 0;
        */
        
        //System.println(Activity.Info.currentHeartRate);
        bpm = Activity.getActivityInfo().currentHeartRate == null? 0: Activity.getActivityInfo().currentHeartRate;


        steps                     = DMode?DSteps:actinfo.steps;
        stepGoal                  = actinfo.stepGoal;
        deltaSteps                = stepGoal - steps;
        stepsReached              = steps.toDouble() / stepGoal;
        kcal                      = DMode?DKcal:actinfo.calories;
        showBpmZones              = App.getApp().getProperty("BpmZones");
        distanceUnit              = Sys.getDeviceSettings().distanceUnits;
        tempUnit                  = 1; //Farhenheight //App.getApp().getProperty("TempUnit");
        ...
        bottomField               = 9; //floors //App.getApp().getProperty("BottomField").toNumber();
        topField                  = 2; //show sensor temperature
        darkUpperBackground       = true; //App.getApp().getProperty("DarkUpperBackground");
        ...
        darkFieldBackground       = true; //App.getApp().getProperty("DarkFieldBackground");
        ...
        invertedBackgColor        = true; 
        apiKey                    = App.getApp().getProperty("DarkSkyApiKey");
                
        var charge                = DMode?DBatt:systemStats.battery;
        var showChargePercentage  = false; //App.getApp().getProperty("ShowChargePercentage");
        var showPercentageUnder20 = true; //App.getApp().getProperty("ShowPercentageUnder20");
        var dayOfWeek             = nowinfo.day_of_week;
        var lcdBackgroundVisible  = darkUpperBackground ? false : true; //App.getApp().getProperty("LcdBackground");
        ...
        var monthAsText           = true; //App.getApp().getProperty("MonthAsText");
        var showCalendarWeek      = false; //App.getApp().getProperty("ShowCalendarWeek");        
        var showMoveBar           = true; //App.getApp().getProperty("ShowMoveBar");
        var showStepBar           = true; //App.getApp().getProperty("ShowStepBar");
        var showCalorieBar        = true; //App.getApp().getProperty("ShowCalorieBar");
        var hourColor             = upperForegroundColor; //App.getApp().getProperty("HourColor").toNumber();
        var minuteColor           = upperForegroundColor; //App.getApp().getProperty("MinuteColor").toNumber();
        var coloredBattery        = App.getApp().getProperty("ColoredBattery");
        var showSunriseSunset     = false; //App.getApp().getProperty("SunriseSunset");
        var activeKcalGoal        = App.getApp().getProperty("ActiveKcalGoal").toNumber();
        var biggerBatteryFont     = false; //App.getApp().getProperty("BiggerBatteryFont");
        var gender;
        var userWeight;
        var userHeight;
        var userAge;
        
        if (DMode) { //My Profile on Debbuging Mode
            gender     = 0; //Male
            userWeight = 89; //195lbs
            userHeight = 180; //6'11"
            userAge    = 36; //years
        }
        else if (profile == null) {
        	//Get Profile from Digital5 App
        	...
        	//Get Profile from Garmin
            gender     = profile.gender; //System.println(gender);
            userWeight = profile.weight / 1000.0;  //System.println(userWeight);
            userHeight = profile.height; //System.println(userHeight);
            userAge    = nowinfo.year - profile.birthYear; //System.println(userAge);
        }

        // Mifflin-St.Jeor Formula (1990)
        var baseKcalMen   = (9.99 * userWeight) + (6.25 * userHeight) - (4.92 * userAge) + 5.0; //System.println(baseKcalMen); // base kcal men
        ...
        kcalReached       = kcal / baseKcal; //System.println(kcalReached);
       ...
       // if (darkFieldBackground) {
            ...
            //dc.fillRectangle(119, 151, 2, 60); //Vertical Middle Line
            
            dc.fillRectangle(0, 0, width, 30); //Top Field Line/Background
            dc.fillRectangle(0, 211, width, 30); //Bottom Field Line/Background
        ...
        var iconOffsetX = 0;
        // Only Show Battery if Charge if below 20%  
        ...
        if (showChargePercentage) { 
            ...
            */
            ...
            var battYOffset = 110; 
            dc.setColor(upperForegroundColor, upperBackgroundColor);        
            dc.drawRectangle(98  + battXOffset, 4 + battYOffset, 8, 16);
            dc.fillRectangle(100 + battXOffset, 3 + battYOffset, 4, 1);
            ...
            dc.fillRectangle(100 + battXOffset, clamp(6, 18, 18 - chargeHeight) + battYOffset, 4, chargeHeight);            
        } 
      //  else { 
       
     //Left of Time Battery   
      /*
      ...
      if (showChargePercentage) { 
            ...
            dc.setColor(upperForegroundColor, upperBackgroundColor);        
            dc.drawRectangle(106 + battXOffset, 8 + battYOffset, 28, 11);
            dc.fillRectangle(134 + battXOffset, 11 + battYOffset, 2, 5);
            ...
            dc.fillRectangle(108 + battXOffset, 10 + battYOffset , clamp (1, 24, 24.0 * charge / 100.0), 7);
        }
        */
        ...
        // Step Bar background
        var arcOffset = 20; //How much from the original location to shift the Steps/Calories Bar
        if (showStepBar) {
            dc.setPenWidth(8);           
            dc.setColor(darkUpperBackground ? Gfx.COLOR_DK_GRAY : Gfx.COLOR_LT_GRAY, upperBackgroundColor);
            for(var i = 0; i < 10 ; i++) {            
                var startAngleLeft  = (130 + arcOffset) + (i * 6);
            ...
            var stopAngleLeft = (184.0 - 59.0 * stepsReached).toNumber() + arcOffset;
            stopAngleLeft     = stopAngleLeft < (130.0 + arcOffset) ? (130.0 + arcOffset) : stopAngleLeft;        
            ...
            for(var i = 0; i < endIndex ; i++) {            
                var startAngleLeft  = (184 + arcOffset) - (i * 6);
                dc.drawArc(centerX, centerY, 117, 0, startAngleLeft, startAngleLeft + 5);
            }
        }
        ...
        if (showCalorieBar) {
            ...
            var stopAngleRight = (-10.0 + 59.0 * kcalReached).toNumber() - arcOffset;
            stopAngleRight = stopAngleRight > (59.0 - arcOffset) ? (59.0 - arcOffset) : stopAngleRight;
            for(var i = 0; i < 10 ; i++) {
                var startAngleRight = (-10 - arcOffset) + (i * 6);
        ...
        // Time Background 88:88
        /*
        if (lcdFont) {
        	//System.println("LCD Background O");
            dc.setColor(DIM_GRAY, upperBackgroundColor);           
            if (showLeadingZero) {
                dc.drawText(centerX, TimeY, digitalUpright72, "88:88", Gfx.TEXT_JUSTIFY_CENTER);
           } else {
                if (is24Hour) {
        ...
        */
        ...
        /*if (is24Hour) {
            ...
            //if (lcdFont) {   
                ...
                //dc.drawText(195, (93 - (51 - TimeY)) , digitalUpright20, amPm, Gfx.TEXT_JUSTIFY_LEFT);  
                dc.drawText(195, (93 - (51 - TimeY)) ,  Graphics.FONT_XTINY, amPm, Gfx.TEXT_JUSTIFY_LEFT);  
        ...
        dc.setColor(upperForegroundColor, upperBackgroundColor);
        var dateYPosition = showMoveBar ? DateY : 119;
        ...
        /*if (onTravel && showHomeTimezone) {
            ...
            dc.drawText(centerX, dateYPosition, Graphics.FONT_SMALL, weekdayText + (monthAsText ? dateText : dateNumberText), Gfx.TEXT_JUSTIFY_CENTER);
        //}
        ...
        switch(upperLeftField) {
            case 0: drawSteps(getXYPositions(UPPER_LEFT), dc, false); break; // I want Steps
        ...
        switch(upperRightField) {
            ...
            case 1: drawCalories(getXYPositions(UPPER_RIGHT), dc, KCAL, UPPER_RIGHT); break; //I want Today Calories
        ...
        switch(lowerLeftField) {
            ...
            case 3: drawHeartRate(getXYPositions(CENTER_CENTER), dc, LOWER_LEFT); break; //I want Heart Rate
        ...
        switch(bottomField) {
            ...
            case 15:
                drawWithUnit(getXYPositions(BOTTOM_FIELD), dc, 15, BOTTOM_FIELD);
                drawCharacter(dc, tempUnit == 0 ? C : F, 0);
                break;
        }
        ...
        dc.setColor(fieldForegroundColor, fieldBackgroundColor);
        switch(topField) {
            case 1: break; //battery
            case 2:
                drawWithUnit(getXYPositions(TOP_TOP), dc, 15, TOP_TOP);
            	var tempUnitYOffset = 206;
            	var tempUnitXOffset = 20;
                dc.setPenWidth(1);
                dc.drawLine(173 - tempUnitXOffset, 216 - tempUnitYOffset, 169 - tempUnitXOffset, 216 - tempUnitYOffset);
                dc.drawLine(169 - tempUnitXOffset, 216 - tempUnitYOffset, 169 - tempUnitXOffset, 223 - tempUnitYOffset);
                dc.drawLine(172 - tempUnitXOffset, 219 - tempUnitYOffset, 168 - tempUnitXOffset, 219 - tempUnitYOffset);
                break;
        }
    ...
    function drawSteps(xyPositions, dc, showDeltaSteps) {       
        ...
        var textXOffset = -80;
        var textYOffset = 30;
        var bmpX  = xyPositions[0] + bmpXOffset;
        var bmpY  = xyPositions[1] + bmpYOffset;
        var textX = xyPositions[2] + textXOffset;
        var textY = xyPositions[3] + textYOffset;
        ...
        //dc.drawText(bmpX, bmpY, Graphics.FONT_SMALL, "Steps", Gfx.TEXT_JUSTIFY_LEFT);

        dc.drawText(textX, textY, lcdFontDataFields ? digitalUpright24 : Graphics.FONT_SMALL, (showDeltaSteps ? deltaSteps * -1 : steps), Gfx.TEXT_JUSTIFY_LEFT);
    }
    ...
    function drawCalories(xyPositions, dc, kcalType, field) {
        ...
        if (lcdFontDataFields) {
            //dc.drawText(bmpX, bmpY, Graphics.FONT_SMALL, "Kcal", Gfx.TEXT_JUSTIFY_RIGHT);
            dc.drawText(field < 4 ? textX : textX + 13, textY, field < 4 ? digitalUpright24 : digitalUpright20, fieldText, Gfx.TEXT_JUSTIFY_RIGHT);
        } else {
            dc.drawText(field < 4 ? textX  :textX + 4, textY, Graphics.FONT_SMALL, fieldText, Gfx.TEXT_JUSTIFY_RIGHT);
        }
    }
    ...
    function drawHeartRate(xyPositions, dc, field) {  
        ...
        dc.setColor(fieldForegroundColor, fieldBackgroundColor);
        if (lcdFontDataFields) {
            dc.drawText(textX, textY, field < 4 ? digitalUpright24 : digitalUpright20, (bpm > 0 ? bpm.toString() : "0"), Gfx.TEXT_JUSTIFY_CENTER);
        } else {
    ...
    function drawWithUnit(xyPositions, dc, sensor, field) {        
        ...
        switch(sensor) {
            ...
            case 13: // Weather
                ...
                break;
            case 15: // Sensor Temperature
                var sensorIter = Toybox.SensorHistory.getTemperatureHistory({});
                var temp = null;
                if (sensorIter != null) {
                    temp = sensorIter.next().data;
                }
                if (tempUnit == 1 && temp != null) { temp = temp * 1.8 + 32; } 
                var icon = 7;
                //icon = App.getApp().getProperty("icon");
                var bmpX  = xyPositions[0];
                var bmpY  = xyPositions[1];
                fieldText = temp != null? temp.format("%.1f"): "--";
            	break;
        }
        dc.setColor(fieldBackgroundColor, Gfx.COLOR_LT_GRAY); 
        //dc.setColor(fieldForegroundColor, fieldBackgroundColor);
        if (lcdFontDataFields) {
    ...
    function drawTime(hourColor, minuteColor, font, dc) {
        ...
        dc.drawText(centerX - 6, lcdFont ? TimeY : y, font, DMode?DHour.format(showLeadingZero ? "%02d" : "%01d"):hour.format(showLeadingZero ? "%02d" : "%01d"), Gfx.TEXT_JUSTIFY_RIGHT);
        dc.setColor(upperForegroundColor, Gfx.COLOR_TRANSPARENT);
        dc.drawText(centerX, lcdFont ? TimeY : y, font, ":", Gfx.TEXT_JUSTIFY_CENTER);
        dc.setColor(minuteColor, Gfx.COLOR_TRANSPARENT);
        dc.drawText(centerX + 6, lcdFont ? TimeY : y, font, DMode?DMinute:clockTime.min.format("%02d"), Gfx.TEXT_JUSTIFY_LEFT);
        dc.setColor(upperForegroundColor, Gfx.COLOR_TRANSPARENT);
    }
